...
class PrivilegeEditForm extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    updateHandler = ( event ) => {
        ...
        const formData = {};
        formData['id'] = this.props.privilege.id
        ...
        axios.put( 'http://localhost:8080/api/privileges', formData)
            .then( response => {
                this.setState( { loading: false } );
                this.props.history.push( '/privileges' );
            } )
            .catch( error => {
                this.setState( { loading: false } );
            } );
    ...
    deleteHandler = ( event ) => {
        ...
        axios.delete( 'http://localhost:8080/api/privileges' + '/' + formData['id'])
            .then( response => {
                this.setState( { loading: false } );
                this.props.history.push( '/privileges' );
            } )
...
export default PrivilegeEditForm;